   ...
   switch (e->tag) {
      ...
      case Iex_Binop: {
         switch (e->Iex.Binop.op) {
            case Iop_8HLto16: {
               ...
               HReg           zHi  = newVRegI(env);
               HReg           zLow = newVRegI(env);
               HReg           sHi  = iselIntExpr_R(env, e->Iex.Binop.arg1);
               HReg           sLow = iselIntExpr_R(env, e->Iex.Binop.arg2);
               LOONGARCH64RI* sh8  = LOONGARCH64RI_I(8, 6, False);
               LOONGARCH64RI* sh56 = LOONGARCH64RI_I(56, 6, False);
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh56, sHi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh56, zHi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh8, zHi, tHi));
               addInstr(
                  env, LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh56, sLow, zLow));
               addInstr(
                  env, LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh56, zLow, tLow));
               addInstr(env, LOONGARCH64Instr_Binary(
                                LAbin_OR, LOONGARCH64RI_R(tHi), tLow, dst));
               return dst;
            }
            case Iop_16HLto32: {
               HReg           dst  = newVRegI(env);
               ...
               HReg           sHi  = iselIntExpr_R(env, e->Iex.Binop.arg1);
               HReg           sLow = iselIntExpr_R(env, e->Iex.Binop.arg2);
               LOONGARCH64RI* sh16 = LOONGARCH64RI_I(16, 6, False);
               LOONGARCH64RI* sh48 = LOONGARCH64RI_I(48, 6, False);
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh48, sHi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh48, zHi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh16, zHi, tHi));
               addInstr(
                  env, LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh48, sLow, zLow));
               addInstr(
                  env, LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh48, zLow, tLow));
               addInstr(env, LOONGARCH64Instr_Binary(
                                LAbin_OR, LOONGARCH64RI_R(tHi), tLow, dst));
               return dst;
            }
            case Iop_32HLto64: {
               ...
               HReg           lo   = iselIntExpr_R(env, e->Iex.Binop.arg2);
               HReg           zHi  = newVRegI(env);
               HReg           zLo  = newVRegI(env);
               LOONGARCH64RI* sh32 = LOONGARCH64RI_I(32, 6, False);
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh32, hi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh32, zHi, zHi));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh32, zHi, dst));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SLLI_D, sh32, lo, zLo));
               addInstr(env,
                        LOONGARCH64Instr_Binary(LAbin_SRLI_D, sh32, zLo, zLo));
               addInstr(env, LOONGARCH64Instr_Binary(
                                LAbin_OR, LOONGARCH64RI_R(zLo), dst, dst));
               return dst;
            }
            case Iop_Add32: {
               HReg            dst = newVRegI(env);
      ...
      case Iex_Unop: {
         switch (e->Iex.Unop.op) {
            ...
            case Iop_1Sto8: case Iop_1Sto16: case Iop_1Sto32: case Iop_1Sto64: {
               HReg           dst = newVRegI(env);
      ...
      case Iex_Binop: {
         switch (e->Iex.Binop.op) {
            ...
            case Iop_ShlV128: case Iop_ShrV128: {
               LOONGARCH64VecBinOp t0Op, t1Op, t2Op, t3Op;
               switch (e->Iex.Binop.op) {
                  case Iop_ShlV128:
                     t0Op = LAvecbin_VBSLL_V;
                     t1Op = LAvecbin_VSLLI_B;
                     t2Op = LAvecbin_VSRLI_B;
                     t3Op = LAvecbin_VBSLL_V;
                     break;
                  case Iop_ShrV128:
                     t0Op = LAvecbin_VBSRL_V;
                     t1Op = LAvecbin_VSRLI_B;
                     t2Op = LAvecbin_VSLLI_B;
                     t3Op = LAvecbin_VBSRL_V;
                     break;
                  default: vassert(0); break;
               }
               ...
               const UChar bytes = shamt / 8;
               const UChar bits  = shamt % 8;
               const HReg  t0    = newVRegV(env);
               addInstr(env,
                        LOONGARCH64Instr_VecBinary(
                           t0Op, LOONGARCH64RI_I(bytes, 5, False), src1, t0));
               if (bits == 0) {
                  return t0;
               }
               ...
               addInstr(env, LOONGARCH64Instr_VecBinary(
                                t1Op, LOONGARCH64RI_I(bits, 3, False), t0, t1));
               addInstr(env,
                        LOONGARCH64Instr_VecBinary(
                           t2Op, LOONGARCH64RI_I(8 - bits, 3, False), t0, t2));
               addInstr(env, LOONGARCH64Instr_VecBinary(
                                t3Op, LOONGARCH64RI_I(1, 5, False), t2, t3));
               addInstr(env, LOONGARCH64Instr_VecBinary(
      ...
      case Iex_Unop: {
         switch (e->Iex.Unop.op) {
            ...
            case Iop_Cnt8x16: {
               HReg dst = newVRegV(env);
               HReg src = iselV128Expr(env, e->Iex.Unop.arg);
               addInstr(env, LOONGARCH64Instr_VecUnary(LAvecun_VPCNT_B, src, dst));
               return dst;
            }
            ...
            case Iop_CmpNEZ128x1: {
               ...
               /*
                * vpickve2gr.d   tmp1, vsrc, 0
                * vpickve2gr.d   tmp2, vsrc, 1
                * or             tmp1, tmp1, tmp2
                * cmpne          tmp1, $zero, tmp1
                * sub            tmp1, $zero, tmp1
                * vreplgr2vr.d   vdst, tmp1
                */
               addInstr(env, LOONGARCH64Instr_VecBinary(
                                LAvecbin_VPICKVE2GR_D,
                                LOONGARCH64RI_I(0, 1, False), src, tmp1));
               addInstr(env, LOONGARCH64Instr_VecBinary(
                                LAvecbin_VPICKVE2GR_D,
                                LOONGARCH64RI_I(1, 1, False), src, tmp2));
               addInstr(env, LOONGARCH64Instr_Binary(
                                LAbin_OR, LOONGARCH64RI_R(tmp2), tmp1, tmp1));
               addInstr(env,
                        LOONGARCH64Instr_Cmp(LAcc_NE, hregZERO(), tmp1, tmp1));
               addInstr(env, LOONGARCH64Instr_Binary(LAbin_SUB_D,
                                                     LOONGARCH64RI_R(tmp1),
                                                     hregZERO(), tmp1));
               addInstr(env, LOONGARCH64Instr_VecUnary(LAvecun_VREPLGR2VR_D,
                                                       tmp1, dst));
               return dst;
